...
class HistoryAggregation:
    ...
    def aggregate(
        ...
        if tf.endswith("m"):
            return HistoryAggregation.aggregate_minutes(
                bars,
                int(tf[:-1]),
            )

        if tf.endswith("h"):
            return HistoryAggregation.aggregate_hours(
                bars,
                int(tf[:-1]),
            )
    ...
    def aggregate_minutes(
        ...
        for bar in bars:
            ...
            HistoryAggregation._merge_bucket(
                buckets,
                ts,
                bar,
            )
    ...
    def aggregate_weekly(
        ...
        for bar in bars:
            ...
            bucket = datetime.fromordinal(monday)
