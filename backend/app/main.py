...
# All chart drawing types (trendlines, rectangles, horizontal lines, price ranges,
# long-position tools, and manual market structure) use this document endpoint.
# The frontend stores normal drawings under the timeframe scope and manual market
# structure under the shared scope for the symbol.
CHART_DRAWINGS_LOCK_FILE = CHART_STORAGE_DIR / ".drawings.lock"
LOCAL_CHART_DRAWINGS_LOCK = threading.RLock()


@contextmanager
def _locked_chart_drawings():
    """Serialize drawing reads/writes across threads and Gunicorn workers."""
    with LOCAL_CHART_DRAWINGS_LOCK:
        lock_handle = CHART_DRAWINGS_LOCK_FILE.open("a+")
        try:
            if fcntl is not None:
                fcntl.flock(lock_handle.fileno(), fcntl.LOCK_EX)
            yield
...
def _chart_drawings_file(symbol: str, scope: str) -> Path:
    safe_symbol = _clean_chart_part(symbol)
    safe_scope = _clean_chart_part(scope)
    return CHART_STORAGE_DIR / f"{safe_symbol}__{safe_scope}__drawings.json"


def _extract_all_drawings(data: Any) -> List[Dict[str, Any]]:
    raw: Any = data
    if isinstance(data, dict):
        if isinstance(data.get("drawings"), list):
    ...
    for item in raw:
        ...
        drawing_id = str(item.get("id") or "").strip()
        drawing_type = str(item.get("type") or "").strip()
        if not drawing_id or not drawing_type or drawing_id in seen_ids:
            continue
...
def _read_chart_drawings_unlocked(symbol: str, scope: str) -> Dict[str, Any]:
    path = _chart_drawings_file(symbol, scope)
    if not path.exists():
        return {
            "symbol": _clean_chart_part(symbol),
            "scope": _clean_chart_part(scope),
    ...
    try:
        ...
        print(f"[chart-drawings] read error {path}: {exc}", flush=True)
        return {
            "symbol": _clean_chart_part(symbol),
            "scope": _clean_chart_part(scope),
            "drawings": [],
    ...
    drawings = _extract_all_drawings(data)
    revision = 0
    updated_at: Any = None
...
def _write_chart_drawings_unlocked(symbol: str, scope: str, data: Any) -> Dict[str, Any]:
    path = _chart_drawings_file(symbol, scope)
    previous = _read_chart_drawings_unlocked(symbol, scope)
    drawings = _extract_all_drawings(data)
    ...
    tmp_file.write_text(json.dumps(stored, indent=2), encoding="utf-8")
    tmp_file.replace(path)
...
def get_chart_drawings(symbol: str, scope: str):
    with _locked_chart_drawings():
        return _read_chart_drawings_unlocked(symbol, scope)


@app.put("/chart/drawings/{symbol}/{scope}")
def put_chart_drawings(symbol: str, scope: str, payload: Any = Body(default={})):
    with _locked_chart_drawings():
        return _write_chart_drawings_unlocked(symbol, scope, payload)
