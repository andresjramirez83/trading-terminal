...
type PendingBullishBreak = {
  ...
  /**
   * Highest record wick in the breakout leg until a real pullback develops.
   */
  candidateHigh: StructureLevel;
  pullbackLow: number;
  pullbackLowestClose: number;
  pullbackBars: number;

  /**
   * Once a meaningful pullback forms, freeze the PRE-pullback HH. A later
   * wick-only recovery is part of the next attempt and must not erase it.
   */
  lockedHigh: StructureLevel | null;
...
type PendingBearishBreak = {
  ...
  candidateLow: StructureLevel;
  pullbackHigh: number;
  pullbackHighestClose: number;
  pullbackBars: number;
  lockedLow: StructureLevel | null;
...
type StructureConfirmationProfile = {
  ...
  /**
   * Absolute percentage floor used with ATR so a one-tick pause on a very
   * quiet symbol cannot create a new structural leg.
   */
  minimumPullbackPricePct: number;
};
...
function getStructureConfirmationProfile(
  ...
  if (timeframeSeconds <= 90) {
    return {
      ...
      minimumPullbackPricePct: 0.0020,
    };
  }
  ...
  if (timeframeSeconds <= 360) {
    return {
      ...
      minimumPullbackPricePct: 0.0025,
    };
  }
  ...
  return {
    ...
    minimumPullbackPricePct: 0.0015,
  };
}

function meaningfulPullbackDistance(
  bars: CleanBar[],
  extremeIndex: number,
  ...
  /**
   * Use ATR from BEFORE the extreme whenever possible. Including the extreme
   * candle itself can massively inflate ATR on small-cap expansion candles and
   * make a very obvious pullback look artificially too small (the JEM case).
   */
  const preExtremeIndex = Math.max(0, extremeIndex - 1);
  const preExtremeAtr = averageTrueRange(bars, preExtremeIndex, 14);
  const fallbackAtr = averageTrueRange(bars, extremeIndex, 14);
  const atr = preExtremeAtr > 0 ? preExtremeAtr : fallbackAtr;
  const price = Math.max(
    0.000001,
    Math.abs(bars[extremeIndex]?.close ?? 0),
  );

  return Math.max(
    atr * atrMultiplier,
    price * profile.minimumPullbackPricePct,
  );
}

function bullishPullbackQualifies(
  bars: CleanBar[],
  pending: PendingBullishBreak,
  profile: StructureConfirmationProfile,
  minimumBars: number,
  atrMultiplier: number,
): boolean {
  const extreme = pending.lockedHigh ?? pending.candidateHigh;
  if (pending.pullbackBars < minimumBars) return false;

  const required = meaningfulPullbackDistance(
    bars,
    extreme.index,
    atrMultiplier,
    profile,
  ...
  const wickDepth = extreme.price - pending.pullbackLow;
  const closeDepth = extreme.price - pending.pullbackLowestClose;
  return wickDepth >= required && closeDepth >= required * 0.45;
}

function bearishPullbackQualifies(
  bars: CleanBar[],
  pending: PendingBearishBreak,
  profile: StructureConfirmationProfile,
  minimumBars: number,
  atrMultiplier: number,
): boolean {
  const extreme = pending.lockedLow ?? pending.candidateLow;
  if (pending.pullbackBars < minimumBars) return false;
  ...
  const wickDepth = pending.pullbackHigh - extreme.price;
  const closeDepth = pending.pullbackHighestClose - extreme.price;
  return wickDepth >= required && closeDepth >= required * 0.45;
}

function updateBullishPendingPullback(
  bars: CleanBar[],
  pending: PendingBullishBreak,
  currentIndex: number,
  profile: StructureConfirmationProfile,
  ...
  /**
   * Before a pullback is locked, a true new record wick still belongs to the
   * same advancing leg. Once lockedHigh is set, later wick-only probes are not
   * allowed to move the old HH candidate.
   */
  if (!pending.lockedHigh && current.high > pending.candidateHigh.price) {
    pending.candidateHigh = {
      index: currentIndex,
      price: current.high,
    };
    pending.pullbackLow = Number.POSITIVE_INFINITY;
    pending.pullbackLowestClose = Number.POSITIVE_INFINITY;
    pending.pullbackBars = 0;
    return;
  }

  pending.pullbackLow = Math.min(pending.pullbackLow, current.low);
  pending.pullbackLowestClose = Math.min(
    pending.pullbackLowestClose,
    current.close,
  );
  pending.pullbackBars += 1;

  if (
    !pending.lockedHigh &&
    bullishPullbackQualifies(
      bars,
      pending,
      profile,
      profile.minimumContinuationPullbackBars,
      profile.continuationPullbackAtrMultiplier,
    )
  ) {
    pending.lockedHigh = { ...pending.candidateHigh };
  }
}

function updateBearishPendingPullback(
  bars: CleanBar[],
  pending: PendingBearishBreak,
  currentIndex: number,
  profile: StructureConfirmationProfile,
  ...
  if (!pending.lockedLow && current.low < pending.candidateLow.price) {
    pending.candidateLow = {
      index: currentIndex,
      price: current.low,
    };
    pending.pullbackHigh = Number.NEGATIVE_INFINITY;
    pending.pullbackHighestClose = Number.NEGATIVE_INFINITY;
    pending.pullbackBars = 0;
    return;
  }

  pending.pullbackHigh = Math.max(pending.pullbackHigh, current.high);
  pending.pullbackHighestClose = Math.max(
    pending.pullbackHighestClose,
    current.close,
  );
  pending.pullbackBars += 1;
  ...
  if (
    ...
    pending.lockedLow = { ...pending.candidateLow };
  }
}
...
function armBullishBreak(
   ...
   * A candle must CLOSE above the previous confirmed HH wick. That confirms
   * exactly one HL. The HL is the
  ...
  state.pendingBullishBreak = {
    ...
    candidateHigh: {
      ...
      price: bars[confirmationIndex].high,
    },
    pullbackLow: Number.POSITIVE_INFINITY,
    pullbackLowestClose: Number.POSITIVE_INFINITY,
    pullbackBars: 0,
    lockedHigh: null,
    transitionOnly,
  };
...
function armBearishBreak(
   ...
   * A close below the previous confirmed LL wick confirms exactly one LH. That LH is the
  ...
  state.pendingBearishBreak = {
    ...
    candidateLow: {
      index: confirmationIndex,
      price: bars[confirmationIndex].low,
    },
    pullbackHigh: Number.NEGATIVE_INFINITY,
    pullbackHighestClose: Number.NEGATIVE_INFINITY,
    pullbackBars: 0,
    lockedLow: null,
    transitionOnly,
  };
...
function tryFinalizeBullishBreak(
  ...
  /**
   * If a pullback was already locked on a previous candle, a CLOSE above the
   * locked wick confirms that old HH immediately. Crucially, a recovery candle
   * may wick above the old HH without moving it. The same current candle can
   * then arm the next bullish break in the main loop and record the true HL.
   */
  if (
    pending.lockedHigh &&
    current.close > pending.lockedHigh.price
  ) {
    const highestWick = pending.lockedHigh;
  ...
  const highestWick = pending.lockedHigh ?? pending.candidateHigh;
  const meaningfulPullback = bullishPullbackQualifies(
    bars,
    pending,
    profile,
    profile.minimumBarsAfterExtreme,
  ...
  if (!meaningfulPullback && !consolidatedAfterExtreme) return;
...
function tryFinalizeBearishBreak(
  ...
  if (
    pending.lockedLow &&
    current.close < pending.lockedLow.price
  ) {
    const lowestWick = pending.lockedLow;
  ...
  const lowestWick = pending.lockedLow ?? pending.candidateLow;
  const meaningfulPullback = bearishPullbackQualifies(
    bars,
    pending,
    profile,
    profile.minimumBarsAfterExtreme,
...
function buildPendingPoints(
  ...
  if (
    ...
    if (state.pendingBullishBreak.confirmationIndex < bars.length) {
      const highest =
        state.pendingBullishBreak.lockedHigh ??
        state.pendingBullishBreak.candidateHigh;
  ...
  if (
    ...
    if (state.pendingBearishBreak.confirmationIndex < bars.length) {
      const lowest =
        state.pendingBearishBreak.lockedLow ??
        state.pendingBearishBreak.candidateLow;
 ...
 *
 * Bullish:
 * - A break is armed only when the candle CLOSES above the previous confirmed
 *   HH wick. A wick-only sweep is not a structural break.
 ...
 *
 * Bearish:
 * - A break is armed only when a candle CLOSES below the previous confirmed
 *   LL wick. A wick-only sweep is not a structural break.
...
export function buildMarketStructure(
  ...
  const confirmationProfile = getStructureConfirmationProfile(bars);
  ...
  for (let index = processingStart; index < bars.length; index += 1) {
    ...
    const bearishWickBreakLevel = state.confirmedLow.price;

    /**
     * Structural breaks are close-confirmed through the previous wick. A wick
     * through HH/LL without a close is liquidity interaction, not a new
     * structural leg. This is the authoritative rule used by demand zones too.
     *
