// src/components/chart/interaction/tools/TrendlineTool.ts
...
export class TrendlineTool implements ChartTool {
  readonly id = "trendline";
  readonly label = "Trendline";
  ...
  onClick(event: ChartMouseEvent, context: ToolContext): boolean | void {
    ...
    const created = this.drawingEngine.createTrendlineFromPoints(
      start,
      end,
      this.getStyle(),
    );
