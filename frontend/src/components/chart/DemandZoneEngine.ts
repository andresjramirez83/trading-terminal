 ...
 * 3. The breakout candle must CLOSE above that previous HH wick. A wick-only
 *    sweep does not validate a structural demand zone.
...
export function buildAutomaticDemandZones(
  ...
  for (const higherHighPoint of bullishLegs) {
     ...
     * - the breakout candle must CLOSE above the previous HH wick.
     *
     * A wick-only sweep is liquidity interaction, not a structural break.
     */
    const completedHigherHigh =
      higherHighPoint.price > previousHighWick;
    const closedAbovePreviousWick =
      confirmationBar.close > previousHighWick;

    if (
      !completedHigherHigh ||
      !closedAbovePreviousWick
    ) {
      continue;
    }
