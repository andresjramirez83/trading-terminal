...
function dedupeScannerRows(items: ScannerRow[]): ScannerRow[] {
  ...
  for (const item of items) {
    ...
    // VWAP +3 can create multiple independent setups for the same symbol
    // during one trading day.  setup_key is unique per frozen projection
    // (symbol + session + freeze time), so preserve every setup instead of
    // collapsing the list to one row per symbol.  Other scanners that do not
    // provide setup_key keep the original one-row-per-symbol behavior.
    const setupKey = String(item.setup_key ?? "").trim();
    const dedupeKey = setupKey ? `setup:${setupKey}` : `symbol:${symbol}`;
